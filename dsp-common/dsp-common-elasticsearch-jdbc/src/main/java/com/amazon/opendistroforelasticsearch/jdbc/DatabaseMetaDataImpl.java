...
public class DatabaseMetaDataImpl implements DatabaseMetaData, JdbcWrapper, LoggingSource {
    ...
    private ElasticsearchConnectionImpl connection;
    private Logger log;

    public DatabaseMetaDataImpl(ElasticsearchConnectionImpl connection, Logger log) {
    ...
    private ResultSet getSchemasX(String catalog, String schemaPattern) throws SQLException {
        ...
        return new ElasticsearchResultSet(null, columnDescriptors, dataRows, log);
    }
    ...
    public ResultSet getCatalogs() throws SQLException {
        ...
        resultSet = new ElasticsearchResultSet(null, columnDescriptors, dataRows, log);
    ...
    private static ElasticsearchResultSet emptyResultSet(Logger log, ResultSetColumnDescriptor... resultSetColumnDescriptors)
            throws SQLException {
        List<List<Object>> rows = new ArrayList<>(0);
        return new ElasticsearchResultSet(null, Arrays.asList(resultSetColumnDescriptors), rows, log);
    }
    ...
    static class ColumnMetadataStatement extends ElasticsearchPreparedStatement {
        // a special statement with custom logic for building the
        // ResultSet it returns on execution

        ColumnMetadataStatement(ElasticsearchConnectionImpl connection, String tableNamePattern, String columnNamePattern, Logger log)
                throws SQLException {
        ...
        static class ColumnMetadataResultSet extends ElasticsearchResultSet {

            ColumnMetadataResultSet(ElasticsearchStatement statement, List<? extends ColumnDescriptor> columnDescriptors,
        ...
        @Override
        protected ElasticsearchResultSet buildResultSet(QueryResponse queryResponse) throws SQLException {
            // enrich/update the resultSet with some JDBC specific data type info
            List<ResultSetColumnDescriptor> columnDescriptors = new ArrayList<>();

            for (ColumnDescriptor cd : queryResponse.getColumns()) {
            ...
            return new ColumnMetadataResultSet(this, columnDescriptors, queryResponse.getRows(), log);
        }
    }
}
