...
public class ElasticsearchStatement implements Statement, JdbcWrapper, LoggingSource {

    protected ElasticsearchConnectionImpl connection;
    protected boolean open = false;
    protected int fetchSize;
    protected ElasticsearchResultSet resultSet;
    protected Logger log;
    private boolean closeOnCompletion;

    public ElasticsearchStatement(ElasticsearchConnectionImpl connection, Logger log) {
    ...
    protected ResultSet executeQueryRequest(JdbcQueryRequest jdbcQueryRequest) throws SQLException {
        ...
        try {
            ...
            if (queryResponse.getError() != null) {
                throw new InternalServerErrorException(queryResponse.getError().getReason(),
                    queryResponse.getError().getType(), queryResponse.getError().getDetails());
            }

            resultSet = buildResultSet(queryResponse);

        } catch (ResponseException e) {
            logAndThrowSQLException(log, new SQLException("Error executing query:" + e.getResponsePayload(), e));
        } catch (IOException e) {
            logAndThrowSQLException(log, new SQLException("Error executing query:" + e.getMessage(), e));
        }
        return resultSet;
    }

    protected ElasticsearchResultSet buildResultSet(QueryResponse queryResponse) throws SQLException {
        return new ElasticsearchResultSet(this, queryResponse, log);
    }
