...
public interface CommonEntityController<ID extends Serializable, T extends CommonEntity<ID>> {
    ...
    @PostMapping("/save")
    @ResponseBody
    default Response<?> save(@ApiParam(value = "待新建记录") @ApiModelFields(requiredFields = {"name"}, filterFields = {
        "id", "enabled", "tenantId", "owner", "createTime", "creator", "lastModifiedTime",
        "lastModifier"}, includeMode = false) @RequestBody T t) {
        if (t == null) {
    ...
    @PostMapping("/update")
    @ResponseBody
    default Response<?> update(@ApiParam(value = "待编辑记录") @ApiModelFields(requiredFields = {"id"}, filterFields = {
        "id", "enabled", "tenantId", "owner", "createTime", "creator", "lastModifiedTime",
        "lastModifier"}, includeMode = false) @RequestBody T t) {
        if (t == null) {
