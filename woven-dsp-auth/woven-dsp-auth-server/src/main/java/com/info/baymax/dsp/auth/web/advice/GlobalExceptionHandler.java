	...
	public Response<?> errorHandler(Exception e, HttpServletRequest request, HttpServletResponse response) {
		...
		if (e != null) {
			...
			if (e instanceof BizException || BizException.class.isAssignableFrom(e.getClass())) {
				 ...
				 * else if (e instanceof AuthenticationException) { CustomOauth2ExceptionHandler handler = new
				 * CustomOauth2ExceptionHandler(); response.setStatus(ErrType.UNAUTHORIZED.getStatus()); result =
				 * handler.handle((AuthenticationException) e); }
				 */
		}
