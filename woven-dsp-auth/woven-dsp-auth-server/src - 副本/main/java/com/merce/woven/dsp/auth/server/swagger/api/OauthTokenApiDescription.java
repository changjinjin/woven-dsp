...
public class OauthTokenApiDescription implements SwaggerAdditionApiDescription {
	...
	public ApiDescription get() {
		return new ApiDescription(AuthConstants.TOKEN_ENTRY_POINT, // url
				"token获取端点", // 描述
	...
	private List<Parameter> parameters() {
		return Arrays.asList(new ParameterBuilder()//
				...
				.required(false)//
				.defaultValue("Basic d2ViX2NsaWVudDoxMjM0NTY=")//
				.build(), 
				/*
				 * new ParameterBuilder()// .description("客户端ID")// .type(new
				 * TypeResolver().resolve(String.class))// .name("client_id")//
				 * .parameterType("query")// .parameterAccess("access")//
				 * .required(false)// .modelRef(new ModelRef("string")) //
				 * .defaultValue("web_client")// .build(), new
				 * ParameterBuilder()// .description("客户端密码")// .type(new
				 * TypeResolver().resolve(String.class))//
				 * .name("client_secret")// .parameterType("query")//
				 * .parameterAccess("access")// .required(false)// .modelRef(new
				...
				new ParameterBuilder()//
						...
						.allowableValues(new AllowableListValues(Arrays.asList("authorization_code", "password",
								"client_credentials", "implicit ", "refresh_token"), "string"))//
