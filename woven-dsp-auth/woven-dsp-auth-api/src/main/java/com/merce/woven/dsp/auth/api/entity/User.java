...
//	@DateTimeFormat(pattern = "yyyy-MM-dd")
	...
	/*
	 * public Collection<? extends GrantedAuthority> getAuthorities() { if (ICollections.hasNoElements(authorities) &&
	 * ICollections.hasElements(roles)) { authorities = new ArrayList<SimpleGrantedAuthority>(); for (Role role : roles)
	 * { Set<Permission> permissions = role.getPermissions(); if (role.getEnabled() == 1 &&
	 * ICollections.hasElements(permissions)) { authorities.addAll(permissions.stream().filter(t -> t.getEnabled() == 1)
	 * .map(t -> new SimpleGrantedAuthority(t.getUrl())).collect(Collectors.toList())); } } } return authorities; }
	 */
