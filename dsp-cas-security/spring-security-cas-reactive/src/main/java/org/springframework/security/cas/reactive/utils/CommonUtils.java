package org.springframework.security.cas.reactive.utils;
...
public final class CommonUtils {
    ...
    /**
     * Determines if a String is blank or not. A String is blank if its empty or if
     * it only contains spaces.
     *
     * @param string the string to check
    ...
    /**
     * Determines if a string is not blank. A string is not blank if it contains at
     ...
     * @param serviceParameterName the name of the parameter that defines the
     *                             service.
    ...
    public static void readAndRespondToProxyReceptorRequest(ServerWebExchange exchange,
                                                            final ProxyGrantingTicketStorage proxyGrantingTicketStorage) {
        ServerHttpRequest request = exchange.getRequest();
        ServerHttpResponse response = exchange.getResponse();

        final String proxyGrantingTicketIou = getParameter(request, PARAM_PROXY_GRANTING_TICKET_IOU);

        final String proxyGrantingTicket = getParameter(request, PARAM_PROXY_GRANTING_TICKET);

        if (CommonUtils.isBlank(proxyGrantingTicket) || CommonUtils.isBlank(proxyGrantingTicketIou)) {
            DataBuffer buffer = response.bufferFactory().wrap("".getBytes(Charset.defaultCharset()));
        ...
        String msg = "<?xml version=\"1.0\"?><casClient:proxySuccess xmlns:casClient=\"http://www.yale.edu/tp/casClient\" />";
        DataBuffer buffer = response.bufferFactory().wrap(msg.getBytes(Charset.defaultCharset()));
        response.writeWith(Mono.just(buffer)).doOnError(error -> DataBufferUtils.release(buffer));
        return;
    }
    ...
    private static boolean requestIsOnStandardPort(final ServerHttpRequest request) {
        final int serverPort = request.getLocalAddress().getPort();
     ...
     * serviceName. Compiles a list of all service parameters for supported
     * protocols and removes them all from the query string.
     *
     * @param request               the HttpServletRequest
     * @param response              the HttpServletResponse
     * @param service               the configured service url (this will be used if
     *                              not null)
     * @param serverNames           the server name to use to construct the service
     *                              url if the service param is empty. Note, prior
     *                              to CAS Client 3.3, this was a single value. As
     *                              of 3.3, it can be a space-separated value. We
     *                              keep it as a single value, but will convert it
     *                              to an array internally to get the matching
     *                              value. This keeps backward compatability with
     *                              anything using this public method.
     * @param artifactParameterName the artifact parameter name to remove (i.e.
     *                              ticket)
     * @param encode                whether to encode the url or not (i.e.
     *                              Jsession).
     * @return the service url to use.
     */
    public static String constructServiceUrl(final ServerWebExchange exchange, final String service,
                                             final String serverNames, final String artifactParameterName, final boolean encode) {
        return constructServiceUrl(exchange, service, serverNames, SERVICE_PARAMETER_NAMES, artifactParameterName,
            encode);
    }
     ...
     *                              value. This keeps backward compatability with
     *                              anything using this public method.
     * @param serviceParameterName  the service parameter name to remove (i.e.
     *                              service)
     * @param artifactParameterName the artifact parameter name to remove (i.e.
     *                              ticket)
     * @param encode                whether to encode the url or not (i.e.
     ...
     */
    public static String constructServiceUrl(final ServerWebExchange exchange, final String service,
                                             final String serverNames, final String serviceParameterName, final String artifactParameterName,
                                             final boolean encode) {
        ServerHttpRequest request = exchange.getRequest();

        if (CommonUtils.isNotBlank(service)) {
            return encode ? urlEncode(service) : service;
        }

        final String serverName = findMatchingServerName(request, serverNames);
        final URIBuilder originalRequestUrl = new URIBuilder(request.getPath().pathWithinApplication().value(), encode);
        originalRequestUrl.setParameters(request.getURI().getRawQuery());

        final URIBuilder builder;
        if (!serverName.startsWith("https://") && !serverName.startsWith("http://")) {
            // FIXME how to check ssl?
            // String scheme = request.isSecure() ? "https://" : "http://";
            SslInfo sslInfo = request.getSslInfo();
            String scheme = sslInfo != null ? "https://" : "http://";
        ...
        if (builder.getPort() == -1 && !requestIsOnStandardPort(request)) {
            builder.setPort(request.getLocalAddress().getPort());
        }

        builder.setEncodedPath(builder.getEncodedPath() + request.getPath().pathWithinApplication().value());
        ...
        final String returnValue = encode ? urlEncode(result) : result;
        LOGGER.debug("serviceUrl generated: {}", returnValue);
        return returnValue;
    }

    /**
     * Safe method for retrieving a parameter from the request without disrupting
     * the reader UNLESS the parameter actually exists in the query string.
     * <p>
     * Note, this does not work for POST Requests for "logoutRequest". It works for
     * all other CAS POST requests because the parameter is ALWAYS in the GET
     * request.
     * <p>
     * If we see the "logoutRequest" parameter we MUST treat it as if calling the
     * standard request.getParameter.
     * <p>
    ...
    public static String safeGetParameter(final ServerHttpRequest request, final String parameter,
        ...
        if (HttpMethod.POST.equals(request.getMethod()) && parameters.contains(parameter)) {
            LOGGER.debug(
                "safeGetParameter called on a POST HttpServletRequest for Restricted Parameters.  Cannot complete check safely.  Reverting to standard behavior for this Parameter");
            return getParameter(request, parameter);
        }
        return null;
        // return request.getQueryString() == null ||
        // !request.getQueryString().contains(parameter) ? null
     ...
     *
     * @param constructedUrl the url to contact.
     * @param factory        connection factory to prepare the URL connection
     *                       instance
     * @param encoding       the encoding to use.
     * @return the response.
     */
    public static String getResponseFromServer(final URL constructedUrl, final HttpURLConnectionFactory factory,
        ...
        try {
            ...
            LOGGER.error("SSL error getting response from host: {} : Error Message: {}", constructedUrl.getHost(),
                e.getMessage(), e);
            throw new RuntimeException(e);
        } catch (final IOException e) {
            LOGGER.error("Error getting response from host: [{}] with path: [{}] and protocol: [{}] Error Message: {}",
                constructedUrl.getHost(), constructedUrl.getPath(), constructedUrl.getProtocol(), e.getMessage(),
                e);
            throw new RuntimeException(e);
        } finally {
            closeQuietly(in);
    ...
    /**
     * Sends the redirect message and captures the exceptions that we can't possibly
     * do anything with.
     *
    ...
    /**
     * Unconditionally close a {@link Closeable}. Equivalent to
     * {@link java.io.Closeable#close()}close(), except any exceptions will be
     ...
     *
     * <p>
     * NOTE: This returns null and will throw a NullPointerException if autoboxed to
     * a boolean.
     * </p>
     *
     * <pre>
    ...
    /**
     * Returns the string as-is, unless it's <code>null</code>; in this case an
