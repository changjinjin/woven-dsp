...
angular.module('sentinelDashboardApp').service('ParamFlowService', ['$http', function ($http) {
  ...
  this.deleteRule = function (entity) {
    ...
    function notValidParamItem(curExItem) {
        ...
        if (isByteClass(curExItem.classType) && 
        		notGoodNumberBetweenExclusive(curExItem.object, -128, 127)) {
            return true;
        }
  ...
  this.checkRuleValid = function (rule) {
      ...
      if (rule.paramIdx === undefined || rule.paramIdx === '' || 
    		  (isNaN(rule.paramIdx) || rule.paramIdx < 0)) {
