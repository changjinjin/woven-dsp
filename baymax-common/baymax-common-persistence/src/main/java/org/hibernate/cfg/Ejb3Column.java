...
public class Ejb3Column {
	...
	public void redefineColumnName(String columnName, String propertyName, boolean applyNamingStrategy) {
		...
		if (applyNamingStrategy) {
			if (StringHelper.isEmpty(columnName)) {
				if (propertyName != null) {
					...
					if (implicitName.getText().contains("_collection&&element_")) {
						implicitName = Identifier.toIdentifier(
								implicitName.getText().replace("_collection&&element_", "_"), implicitName.isQuoted());
					}

					final Identifier physicalName = physicalNamingStrategy.toPhysicalColumnName(implicitName,
				...
				final Identifier physicalName = physicalNamingStrategy.toPhysicalColumnName(explicitName,
						database.getJdbcEnvironment());
				mappingColumn.setName(physicalName.render(database.getDialect()));
			}
		} else {
	...
	/**
	 * Find appropriate table of the column. It can come from a secondary table or from the main table of the persistent
	 * class
	 *
	...
	public Join getJoin() {
		...
		if (join == null) {
			throw new AnnotationException("Cannot find the expected secondary table: no " + explicitTableName
					+ " available for " + propertyHolder.getClassName());
		}
	...
	private static Ejb3Column[] buildImplicitColumn(PropertyData inferredData, String suffixForDefaultColumnName,
		...
		if (nullability != Nullability.FORCED_NULL && inferredData.getClassOrElement().isPrimitive()
