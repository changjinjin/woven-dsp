...
public class JestServiceImpl implements JestService {
    ...
    public JestResult updateDocument(String script, String index, String type, String id) {
         ...
         * "        \"source\" : \""+article.getSource()+"\"," + "        \"url\" : \""+article.getUrl()+"\"," +
    ...
    @Override
    public <T> JestResult getDocument(Class<T> clazz, String index, String type, String id) {
        Get get = new Get.Builder(index, id).type(type).build();
        JestResult result = null;
        try {
            result = jestClient.execute(get);
            T o = (T) result.getSourceAsObject(clazz);
    ...
    /*
     * @Override public List<Suggestion> suggest() { String suggestionName = "my-suggestion"; Suggest suggest = new
     * Suggest.Builder( "{" + "  \"" + suggestionName + "\" : {" + "    \"text\" : \"the amsterdma meetpu\"," +
     * "    \"term\" : {" + "      \"field\" : \"body\"" + "    }" + "  }" + "}").build(); SuggestResult suggestResult =
     * null; List<SuggestResult.Suggestion> suggestionList = null; try { suggestResult = jestClient.execute(suggest);
     * log.info("suggestResult.isSucceeded() == " + suggestResult.isSucceeded()); suggestionList =
     * suggestResult.getSuggestions(suggestionName); log.info("suggestionList.size() == " + suggestionList.size()); for
     * (SuggestResult.Suggestion suggestion : suggestionList) { System.out.println(suggestion.text); } } catch
     * (IOException e) { e.printStackTrace(); } return suggestionList; }
     */

    @SuppressWarnings("unchecked")
    @Override
    public <T> List<Hit<T, Void>> searchAll(String index, Class<T> clazz) {
    ...
    @SuppressWarnings("unchecked")
    @Override
    public <T> List<Hit<T, Void>> createSearch(String keyWord, String type, Class<T> clazz, String... fields) {
        SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
    ...
    public JsonObject searchEvent(String param) {
        JsonObject returnData = JsonParser.parseString(param).getAsJsonObject();
    ...
    public JsonObject searchInitEventHistogram(String param) {
        JsonObject returnData = JsonParser.parseString(param).getAsJsonObject();
        Search search = new Search.Builder(returnData.toString()).addIndex("pi").addType("event")
