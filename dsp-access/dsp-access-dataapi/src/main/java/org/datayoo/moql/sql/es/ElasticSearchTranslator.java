...
public class ElasticSearchTranslator implements SqlTranslator {
    ...
    {
        functionTranslators.put(MatchTranslator.FUNCTION_NAME, new MatchTranslator());
        functionTranslators.put(MatchPhraseTranslator.FUNCTION_NAME, new MatchPhraseTranslator());
        functionTranslators.put(MatchPhrasePrefixTranslator.FUNCTION_NAME, new MatchPhrasePrefixTranslator());
        functionTranslators.put(TermsSetTranslator.FUNCTION_NAME, new TermsSetTranslator());
        functionTranslators.put(Regex.FUNCTION_NAME, new RegExpTranslator());
        functionTranslators.put(FuzzyTranslator.FUNCTION_NAME, new FuzzyTranslator());
        ...
        functionTranslators.put(MoreLikeTranslator.FUNCTION_NAME, new MoreLikeTranslator());
    }
    ...
    protected void translate2Query(SelectorImpl selector, JsonObject jsonObject,
        ...
        translateSelectClause(selector.getSelectorDefinition(), jsonObject, translationContext);
    ...
    protected void translateSelectClause(SelectorDefinition selectorDefinition, JsonObject jsonObject,
                                         Map<String, Object> translationContext) {
    ...
    protected void translateLimitClause(Limit limit, JsonObject jsonObject, Map<String, Object> translationContext) {
        ...
        Object[] searchAfter = (Object[]) translationContext.get(EsTranslationContextConstants.RESULT_SORT_FEATURES);
    ...
    protected void translate2Aggregations(SelectorImpl selector, JsonObject jsonObject,
                                          Map<String, Object> translationContext) {

        RecordSetOperator recordSetOperator = selector.getRecordSetOperator();
        if (recordSetOperator instanceof Group) {
            translate2GroupAggregations((GroupRecordSetOperator) recordSetOperator, jsonObject, selector.getHaving(),
                selector.getLimit(), selector.getOrder(), translationContext);
        } else if (recordSetOperator.getColumns().getColumnsMetadata().isDistinct()) {
            translate2DistinctAggregations((ColumnsRecordSetOperator) recordSetOperator, jsonObject,
                selector.getLimit(), selector.getOrder(), translationContext);
        } else {
            translateColumnAggregations((ColumnsRecordSetOperator) recordSetOperator, jsonObject, translationContext);
        }
    }

    protected void translate2GroupAggregations(GroupRecordSetOperator groupRecordSetOperator, JsonObject jsonObject,
                                               Having having, Limit limit, Order order, Map<String, Object> translationContext) {
        ...
        for (int i = 0; i < columns.length; i++) {
            baseObject = translate2TermsAggs(columns[i], baseObject, size, order, translationContext);
            size = 0;
        }
        ...
        JsonObject havingFilter = null;
        JsonObject bucket_selector = null;
        JsonObject buckets_path = null;
        JsonObject script = null;
        if (having != null) {
            havingFilter = new JsonObject();
            bucket_selector = new JsonObject();
            buckets_path = new JsonObject();
        ...
        for (int i = 0; i < columns.length; i++) {
            ...
            AggregationFunction operand = (AggregationFunction) columns[i].getOperand();
            translateFunctionAggregation(operand, aggregation, translationContext);
            ...
            if (order != null) {
                ...
                if (orderType != null) {
                    ...
                    if (operand instanceof Count) {
                        orderObject.addProperty("_count", orderType.name().toLowerCase());
                    } else {
            ...
            if (having != null) {
                ...
                // orderArray.add(orderObject);
                // }
                ConditionMetadata havingMetadata = having.getHavingMetadata();
                OperationMetadata operation = havingMetadata.getOperation();
                String operator = operation.getOperator();
                OperatorType operatorType = operation.getOperatorType();
            }
        }
    ...
    protected JsonObject translate2TermsAggs(Column column, JsonObject jsonObject, int size, Order order,
                                             Map<String, Object> translationContext) {
        JsonObject aggsObject = new JsonObject();
        jsonObject.add("aggs", aggsObject);
        JsonObject aggregation = new JsonObject();
        translateTermsAggregation(getOperandName(column.getOperand()), aggregation, size, order, translationContext);
        aggsObject.add(getOperandName(column.getOperand()), aggregation);
        return aggregation;
    }

    protected void translateFunctionAggregation(AggregationFunction aggregationFunction, JsonObject jsonObject,
                                                Map<String, Object> translationContext) {
    ...
    protected void translateTermsAggregation(String fieldName, JsonObject jsonObject, int size, Order order,
                                             Map<String, Object> translationContext) {
        JsonObject termsObject = new JsonObject();
        termsObject.addProperty("field", fieldName);
        if (size != 0)
            termsObject.addProperty("size", size);
        JsonArray orderArray = translateTermsOrder(fieldName, order, translationContext);
        if (orderArray != null)
    ...
    protected void translate2DistinctAggregations(ColumnsRecordSetOperator columnsRecordSetOperator,
                                                  JsonObject jsonObject, Limit limit, Order order, Map<String, Object> translationContext) {
        JsonObject baseObject = jsonObject;
        int size = getLimitSize(limit);
        for (Column column : columnsRecordSetOperator.getColumns().getColumns()) {
            baseObject = translate2TermsAggs(column, baseObject, size, order, translationContext);
            size = 0;
        }
    }

    protected void translateColumnAggregations(ColumnsRecordSetOperator columnsRecordSetOperator, JsonObject jsonObject,
                                               Map<String, Object> translationContext) {
        for (Column column : columnsRecordSetOperator.getColumns().getColumns()) {
            JsonObject aggsObject = new JsonObject();
            jsonObject.add("aggs", aggsObject);
            if (column.getOperand() instanceof AggregationFunction) {
                JsonObject aggregation = new JsonObject();
                translateFunctionAggregation((AggregationFunction) column.getOperand(), aggregation,
                    translationContext);
    ...
    protected void translate2CommonQuery(SelectorImpl selector, JsonObject jsonObject,
                                         Map<String, Object> translationContext) {
        ...
        if (selector.getHaving() != null) {
            ...
            if (selector.getWhere() != null) {
                jsonElement = translateWhereClause(selector.getWhere(), queryObject, true, translationContext);
            } else {
                jsonElement = new JsonObject();
                queryObject.add("bool", jsonElement);
            }

            // 非聚合是having条件可以转义成filter，聚合条件需要转化成BucketSelectorPipelineAggregationBuilder(Bucket Selector Aggregation)
            if (!isAggregations(selector)) {
                translateHavingClause((HavingImpl) selector.getHaving(), jsonElement, translationContext);
            }
        } else {
            translateWhereClause(selector.getWhere(), queryObject, false, translationContext);
        }
    }
    ...
    protected JsonElement translateWhereClause(Condition condition, JsonElement jsonElement, boolean hasHaving,
                                               Map<String, Object> translationContext) {
        if (condition == null) {
            if (!hasHaving) {
    ...
    protected JsonElement shellLogicExpression(Operand operand, JsonElement jsonElement,
                                               Map<String, Object> translationContext) {
    ...
    protected void translateOperand(Operand operand, JsonElement jsonElement, boolean having,
                                    Map<String, Object> translationContext) {
        if (operand instanceof AbstractOperationExpression) {
            AbstractOperationExpression expression = (AbstractOperationExpression) operand;
            if (expression.getExpressionType() == ExpressionType.LOGIC) {
                translateLogicExpression(expression, jsonElement, having, translationContext);
            } else if (expression.getExpressionType() == ExpressionType.RELATION) {
                translateRelationExpression(expression, jsonElement, having, translationContext);
            } else if (expression.getExpressionType() == ExpressionType.ARITHMETIC) {
                throw new MoqlTranslationException(StringFormater.format("The expression '{}' does not support!",
            ...
            translateParenExpression(parenExpression, jsonElement, having, translationContext);
            ...
            throw new MoqlTranslationException(
                StringFormater.format("The operand '{}' does not support!", operand.getOperandType().toString()));
        }
    }
    ...
    protected void translateLogicExpression(AbstractOperationExpression expression, JsonElement jsonElement,
        ...
        if (expression.getOperator() == LogicOperator.NOT) {
            translateNotExpression((NotExpression) expression, boolObject, having, translationContext);
        } else {
            if (expression.getOperator() == LogicOperator.AND) {
                ...
                translateLogicBinaryExpression(operator, expression.getLeftOperand(), expression.getRightOperand(),
                    boolObject, having, translationContext);
            } else {
                translateLogicBinaryExpression("should", expression.getLeftOperand(), expression.getRightOperand(),
                    boolObject, having, translationContext);
            }
        }
        putObject(jsonElement, "bool", boolObject);
    ...
    protected void translateNotExpression(NotExpression expression, JsonElement jsonElement, boolean having,
                                          Map<String, Object> translationContext) {
        ...
        if (expression.getRightOperand() instanceof IsExpression) {
            ...
            translateIsExpression(isExpression.getLeftOperand(), jsonElement, true, translationContext);
        } else {
            translateOperand(expression.getRightOperand(), not, having, translationContext);
            putObject(jsonElement, "must_not", not);
        }
    }

    protected void translateLogicBinaryExpression(String operator, Operand lOperand, Operand rOperand,
                                                  JsonElement jsonElement, boolean having, Map<String, Object> translationContext) {
    ...
    protected void translateRelationExpression(AbstractOperationExpression expression, JsonElement jsonElement,
        ...
        if (expression.getOperator() == RelationOperator.EQ) {
            translateEQExpression(expression.getLeftOperand(), expression.getRightOperand(), jsonElement, having,
                translationContext);
        } else if (expression.getOperator() == RelationOperator.GT) {
            translateLGExpression("gt", expression.getLeftOperand(), expression.getRightOperand(), jsonElement,
                translationContext);
        } else if (expression.getOperator() == RelationOperator.GE) {
            translateLGExpression("gte", expression.getLeftOperand(), expression.getRightOperand(), jsonElement,
                translationContext);
        } else if (expression.getOperator() == RelationOperator.LT) {
            translateLGExpression("lt", expression.getLeftOperand(), expression.getRightOperand(), jsonElement,
                translationContext);
        } else if (expression.getOperator() == RelationOperator.LE) {
            translateLGExpression("lte", expression.getLeftOperand(), expression.getRightOperand(), jsonElement,
                translationContext);
        } else if (expression.getOperator() == RelationOperator.NE) {
            translateNEExpression(expression.getLeftOperand(), expression.getRightOperand(), jsonElement,
                translationContext);
        } else if (expression.getOperator() == RelationOperator.BETWEEN) {
            translateBetweenExpression((BetweenExpression) expression, jsonElement, translationContext);
        } else if (expression.getOperator() == RelationOperator.LIKE) {
            translateLikeExpression(expression.getLeftOperand(), expression.getRightOperand(), jsonElement,
                translationContext);
        } else if (expression.getOperator() == RelationOperator.IN) {
            translateInExpression((InExpression) expression, jsonElement, translationContext);
        } else if (expression.getOperator() == RelationOperator.IS) {
            translateIsExpression(expression.getLeftOperand(), jsonElement, false, translationContext);
            ...
            translateOperand(expression.getRightOperand(), jsonElement, having, translationContext);
        }
    }

    protected void translateParenExpression(ParenExpression expression, JsonElement jsonElement, boolean having,
                                            Map<String, Object> translationContext) {
        translateOperand(expression.getOperand(), jsonElement, having, translationContext);
    }

    protected void translateEQExpression(Operand lOperand, Operand rOperand, JsonElement jsonElement, boolean having,
                                         Map<String, Object> translationContext) {
    ...
    protected void translateLGExpression(String operator, Operand lOperand, Operand rOperand, JsonElement jsonElement,
                                         Map<String, Object> translationContext) {
    ...
    protected void translateNEExpression(Operand lOperand, Operand rOperand, JsonElement jsonElement,
                                         Map<String, Object> translationContext) {
        JsonObject boolObject = new JsonObject();
    ...
    protected void translateLikeExpression(Operand lOperand, Operand rOperand, JsonElement jsonElement,
        ...
        regex.addProperty(getOperandName(lOperand), LikeExpression.translatePattern2Regex(getOperandName(rOperand)));
        putObject(jsonElement, "regexp", regex);
    }

    protected void translateInExpression(InExpression expression, JsonElement jsonElement,
                                         Map<String, Object> translationContext) {
    ...
    protected void translateFunction(AbstractFunction function, JsonElement jsonObject,
                                     Map<String, Object> translationContext) {
        ESFunctionTranslator functionTranslator = functionTranslators.get(function.getName());
        if (functionTranslator == null) {
            throw new MoqlTranslationException(
                StringFormater.format("The function '{}' does not support!", function.getName()));
        } else {
    ...
    protected void translateHavingClause(HavingImpl having, JsonElement jsonElement,
        ...
        translateOperand(having.getCondition().getOperand(), filterObject, true, translationContext);
