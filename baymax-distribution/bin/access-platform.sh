...
#JVM_OPTS="${JVM_OPTS} -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=8603"
JVM_OPTS="${JVM_OPTS} -DWOVEN_COM_NAME=${WOVEN_COM_NAME} -Dfile.encoding=UTF-8 -DWOVEN_APP=woven -DWOVEN_LOGS_DIR=${WOVEN_LOGS_DIR} ${ACCESS_PLATFORM_JVM_OPTS}"
JVM_OPTS="${JVM_OPTS} -classpath ${WOVEN_CONF}:${YARN_CONF_DIR}:lib/*:${ALL_EXTRA_CLASSPATH:-.} com.info.baymax.dsp.access.platform.DspPlatformStarter"
