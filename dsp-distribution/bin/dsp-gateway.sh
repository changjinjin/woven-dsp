...
JVM_OPTS="${JVM_OPTS} -DWOVEN_COM_NAME=${WOVEN_COM_NAME} -Dfile.encoding=UTF-8 -DWOVEN_APP=woven -DWOVEN_LOGS_DIR=${WOVEN_LOGS_DIR} -DKAFKA_BROKER_LIST=${KAFKA_BROKER_LIST} -Dgrpc.server.address=${GRPC_WOVEN_DFEXECUTOR_ADDR} -Dgrpc.server.port=${GRPC_WOVEN_DFEXECUTOR_PORT} ${DSP_GATEWAY_JVM_OPTS}"
JVM_OPTS="${JVM_OPTS} -classpath ${WOVEN_CONF}:${YARN_CONF_DIR}:lib/*:${SPARK_HOME}/jars/*:${ALL_EXTRA_CLASSPATH:-.} com.info.baymax.dsp.gateway.GatewayStarter"
